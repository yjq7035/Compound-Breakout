...
--   4. 营地系统 h9Z4->hlc4 30s 刷新 上限 4+在线 初始 2
--   5. Boss 系统 h31v(1 号)/hqR6(2 号)/h01e(3 号) + hbaj 仆从宫格（分阶段创建防隔墙偷打）
--   6. 关卡触发：营地摧毁/Boss 击杀 移除对应墙，3 号后 350 区域全员进入通关
--   7. 分阶段流程：start 仅创建 1 号内容；1 号击杀后创建 2 号内容；2 号击杀后创建 3 号内容（墙体 5 个坐标保持不变）
--
...
--   6. -8626.4,  -15303.5
-- Boss 坐标（2026-08-24）：
--   1 号 Boss -14868.4, -10349.3
--   2 号 Boss -10250.5, -14186.0
--   3 号 Boss -8835.9,  -14844.6
-- 复活/商店坐标（2026-08-24）：
--   复活点 -11787.8, -14967.1
--   药剂商店 -11388.4, -14565.7
-- Boss 仆从（2026-08-24 fix 2026-08-24）：
--   1 号 以 -14300.2,-10995.4 为中心 3x3=9 个 间隔 200
--        中间行以中心为基准 左右各 200；上行 y+200/下行 y-200 同样 3 个
--   2 号 以 boss 面向前 100 码为起点，宫格 5 个（3 个 h11q+2 个 hA11）：起点 3 个（左中右），推前 35 再 2 个（左右）
--   3 号 以 boss 为中心宫格 N 个 间隔 35
-- 刷怪点（2026-08-25 新增 2026-08-25 调整）：
--   刷怪点 1 中心 -12880.6,-11384.0 6 单位 间隔 90（非满 9 宫格）
--     上行 y+90 3 个 h11q；中行 y 两边 hA11（2 个）；下行 y-90 1 个 h00p（居中）
--   刷怪点 2 中心 -11990.7,-12169.0 6 单位 间隔 90
--     中心 1 个 nl3r；x+90 1 个 n611 + x+90 y+90 1 个 n611 + x+90 y-90 1 个 n611；y-90 1 个 n642 + y+90 1 个 n642
--   刷怪点 3 中心 -12171.7,-13688.3 2 单位 间隔 90 两边各 1 个 hlc4
--   刷怪点 4 中心 -11148.1,-12564.9 2 单位 间隔 90 左右各 1 个 h11q
-- 触发墙（用户给出大概坐标，自动最近匹配）：
--   营地 1 -11582.7,-12989 -> 横墙 -11162.2,-12736.6 (最近 横墙 1)
--   营地 2 -11962.0,-10687.5 -> 竖墙 -13307.5,-11940.9 (最近 竖墙 1)
--   1 号 Boss 击杀 -> 竖墙 -11384.7,-14049.5 (最近 竖墙 2)
--   2 号 Boss 击杀 -> 竖墙 -9939.2,-14109.1 (最近 竖墙 3)
--   3 号 Boss 击杀 -> 竖墙 -9283.2,  -15104.3 (最近 竖墙 4)
-- 通关区域：-9629.7,-15068.3 为中心 350x350
--
-- 调用：
--   require "Game.Layers.Layer1"
--   Layer1.start()             -- 关卡 1 启动（墙/营地/Boss/事件）
--   Layer1.shutdown()          -- 关卡 1 关闭清理
-- ============================================================
...
Layer1.walls = {
    { x = -11147.3, y = -12794.7, id = "B000", dir = "H", face = 270, name = "横墙 1" },
    { x = -13345.1, y = -11943.7, id = "DL84", dir = "V", face = 0,   name = "竖墙 1" },
    { x = -11393.4, y = -14058.5, id = "DL84", dir = "V", face = 0,   name = "竖墙 2" },
    { x = -9975.7,  y = -14137.8, id = "DL84", dir = "V", face = 0,   name = "竖墙 3" },
    { x = -9350.8,  y = -15071.1, id = "DL84", dir = "V", face = 0,   name = "竖墙 4" },
}
...
-- ------------------------------------------------------------
Layer1.camps = {
    { x = -11578.3, y = -12966.9, name = "营地 1" },
    { x = -12011.9, y = -10722.1, name = "营地 2" },
    { x = -13559.1, y = -10209.1, name = "营地 3" },
    { x = -15028.4, y = -12065.9, name = "营地 4" },
    { x = -10819.0, y = -14009.4, name = "营地 5" },
    { x = -8626.4,  y = -15303.5, name = "营地 6" },
}
Layer1Camps = Layer1.camps
Layer1CampPos = Layer1.camps

Layer1.bosses = {
    { x = -14868.4, y = -10349.3, name = "1 号 Boss" },
    { x = -10250.5, y = -14186.0, name = "2 号 Boss" },
    { x = -8835.9,  y = -14844.6, name = "3 号 Boss" },
}
...
Layer1.bossMinions = {
    [1] = { center = { x = -14300.2, y = -10995.4 }, spacing = 200, count = 9, name = "1 号仆从" },
    [2] = { forward = 100, spacing = 35, count = 5, name = "2 号仆从", ids = { "h11q", "h11q", "h11q", "hA11", "hA11" } },
    [3] = { center = "boss", spacing = 35, name = "3 号仆从" },
}
Layer1BossMinions = Layer1.bossMinions

-- 新增刷怪点（2026-08-25）
Layer1.mobSpawnPos = { x = -12880.6, y = -11384.0, spacing = 90, name = "刷怪点 1" }
Layer1.mobSpawnUnits = {} -- 运行时句柄列表
Layer1MobSpawnPos = Layer1.mobSpawnPos
-- 刷怪点 2（2026-08-25 新增）
Layer1.mobSpawnPos2 = { x = -11990.7, y = -12169.0, spacing = 90, name = "刷怪点 2" }
Layer1.mobSpawnUnits2 = {}
Layer1MobSpawnPos2 = Layer1.mobSpawnPos2
-- 刷怪点 3（2026-08-25 新增）
Layer1.mobSpawnPos3 = { x = -12171.7, y = -13688.3, spacing = 90, name = "刷怪点 3" }
Layer1.mobSpawnUnits3 = {}
Layer1MobSpawnPos3 = Layer1.mobSpawnPos3
-- 刷怪点 4（2026-08-25 新增）
Layer1.mobSpawnPos4 = { x = -11148.1, y = -12564.9, spacing = 90, name = "刷怪点 4" }
...
Layer1.handles = {}
Layer1.campData = {}
Layer1.campTimer = nil -- 唯一 true 计时器驱动所有营地（每 30s 检查）
Layer1.boss1Unit = nil
Layer1.boss1Minions = {}
Layer1.boss2Minions = {}
Layer1.boss3Minions = {}
Layer1.bossUnits = {} -- [1..3] = Unit
Layer1.bossStage = 1 -- 1=仅 1 号已创建 2=2 号已创建 3=3 号已创建
Layer1.boss1ChaseTimer = nil -- 1 号 Boss 死后仆从追击计时器
Layer1.bossKilled = {} -- [1..3] = true 已击杀去重，防止同一 Boss 死亡回调触发两次
...
Layer1.triggerWalls = {
    camp1 = { tx = -11162.2, ty = -12736.6, desc = "营地 1 摧毁->横墙" },
    camp2 = { tx = -13307.5, ty = -11940.9, desc = "营地 2 摧毁->竖墙 1" },
    boss1 = { tx = -11384.7, ty = -14049.5, desc = "1 号 Boss 击杀->竖墙 2" },
    boss2 = { tx = -9939.2,  ty = -14109.1, desc = "2 号 Boss 击杀->竖墙 3" },
    boss3 = { tx = -9283.2,  ty = -15104.3, desc = "3 号 Boss 击杀->竖墙 4" },
}
-- 通关区域
...
function Layer1.createWalls()
    if #Layer1.handles > 0 then return Layer1.handles end
...
-- Boss 仆从宫格公式
-- ============================================================
-- 1 号 Boss 专用：以中心为中间行，中心左右 200，上行 y+200/下行 y-200 各 3 个，共 9 个
...
function Layer1.calcBoss2ForwardGrid(bossX, bossY, facingDeg, forward, spacing)
    ...
    -- 5 个：起点行 3 个（左/中/右），前推一行 2 个（左右） -> 3 个 h11q + 2 个 hA11
    return {
...
-- 刷怪点 2 6 单位（2026-08-25 新增）：中心 nl3r + 东侧 3 个 n611 + 南北 2 个 n642
...
-- 刷怪点 3 2 单位（2026-08-25 新增）：中心两边各 1 个 hlc4
function Layer1.calcMobSpawnGrid3(cx, cy, spacing)
    ...
    local positions = {}
    -- 两边各 1 个 hlc4：x-90 与 x+90，y 保持中心
...
function Layer1.createCamps()
    if #Layer1.campData > 0 then return Layer1.campData end
    local p = getEnemyPlayer()
    ...
    -- 唯一计时器驱动所有营地：每 30s 检查各营地绑定数量，未达上限则在营地位置创建狱卒（走单核内核，保证同步）
...
function Layer1.createBoss1()
    if Layer1.boss1Unit and isUnitAlive(Layer1.boss1Unit) then return Layer1.boss1Unit end
...
-- 1 号 Boss 死亡时：存活仆从转为追击玩家（扩大索敌 + 攻击最近英雄），不再直接删除
function Layer1.activateBoss1MinionsChase()
    ...
    -- 周期性重新索敌（每 3 秒），防止丢失目标后发呆；无存活时自动停止
...
-- 2 号 Boss：仅在 1 号击杀后创建，避免隔墙被远程命中
function Layer1.createBoss2()
    if Layer1.bossUnits[2] and isUnitAlive(Layer1.bossUnits[2]) then return Layer1.bossUnits[2] end
...
function Layer1.spawnBoss2Minions(facingDeg)
    -- 2 号仆从：以 boss 面向前 100 码为起点，宫格 5 个 3*h11q+2*hA11
...
-- 3 号 Boss：仅在 2 号击杀后创建
function Layer1.createBoss3()
    if Layer1.bossUnits[3] and isUnitAlive(Layer1.bossUnits[3]) then return Layer1.bossUnits[3] end
...
-- 兼容旧接口：仅创建 1 号（分阶段后 2/3 号不再随 start 创建）
...
local function registerLayerEvents()
    ...
    local deathEv = Event:new(nil, EVENT_PLAYER_UNIT_DEATH, function(ev)
        ...
        -- Boss 击杀：按 Boss 坐标距离判定是几号 + 必须匹配对应 bossId，防止仆从/野怪在 Boss 附近死亡误触发
        ...
        -- 严格校验：死亡单位类型必须等于对应 BossId，否则视为误判（hbaj 仆从在 Boss 点附近死亡会进距离 500）
        ...
        -- 去重：同一 Boss 已击杀过则不再执行，防止引擎/重复注册导致触发两次
        ...
        if killerBossIdx == 1 then
            ...
            if not Layer1.bossUnits[2] or not isUnitAlive(Layer1.bossUnits[2]) then
                ...
                if SystemMessage and SystemMessage.send then
                    SystemMessage.send({{"STR", "2 号 Boss 已出现！", SystemMessage.COLOR_WARN}}, 3.0)
                else
                    Player.sendAll("2 号 Boss 已出现！")
                end
            end
        elseif killerBossIdx == 2 then
            ...
            if not Layer1.bossUnits[3] or not isUnitAlive(Layer1.bossUnits[3]) then
                ...
                if SystemMessage and SystemMessage.send then
                    SystemMessage.send({{"STR", "3 号 Boss 已出现！", SystemMessage.COLOR_WARN}}, 3.0)
                else
                    Player.sendAll("3 号 Boss 已出现！")
                end
            end
...
-- 启动 / 关闭（分阶段：初始仅创建 1 号 Boss 内容，2/3 号击杀后依次创建，墙体 5 个保持不变）
function Layer1.start()
    if Layer1.started then return end
    Layer1.started = true; Layer1.finished = false
    Layer1.bossStage = 1
    Layer1.bossKilled = {}
    Layer1.createWalls() -- 5 墙保持不变
    Layer1.createCamps()
    -- 初始阶段仅 1 号 Boss，避免远程隔墙打 2/3 号
    Layer1.createBoss1()
    Layer1.spawnBoss1Minions(9)
    -- 刷怪点 1 -12880.6,-11384.0 上行 y+90 3*h11q 中行 y 两边 hA11 下行 y-90 1*h00p
    Layer1.spawnMobSpawn()
    -- 刷怪点 2 -11990.7,-12169.0 中心 nl3r 东侧 3*n611 南北 2*n642
    Layer1.spawnMobSpawn2()
    -- 刷怪点 3 -12171.7,-13688.3 两边各 1 个 hlc4
    Layer1.spawnMobSpawn3()
    -- 刷怪点 4 -11148.1,-12564.9 左右各 1 个 h11q
    Layer1.spawnMobSpawn4()
    registerLayerEvents()
end
